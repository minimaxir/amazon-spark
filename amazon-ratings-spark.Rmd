...
# do not cache the initial data set; only cache the final result
...
Verify latter query worked correctly, and table cached correctly, by filtering on specific values:
...
plot <- ggplot(temp, aes(x=avg_rating, y=norm, fill=category)) +
          geom_density(stat="identity", alpha=0.75, bw=0.2, size=0.25) +
          fte_theme() +
...
plot <- ggplot(df_breakdown_users, aes(x=user_nth, y=count, fill=as.factor(rating))) +
  ...
  theme(legend.title = element_blank(), legend.position="top", legend.direction="horizontal", legend.key.width=unit(0.5, "cm"), legend.key.height=unit(0.25, "cm")) +
          labs(title="Distribution of Average User Ratings for Amazon Products", x="Average User Ratings for Amazon Products (5 Ratings Minimum)", y="# of Users")
...
plot <- ggplot(df_breakdown_time, aes(x=date, y=count, fill=as.factor(rating))) +
          geom_bar(stat="identity", position = "fill", width=32) +
          fte_theme() +
...
max_save(plot, "category_breakdown", "Amazon")
```
...
Removed because Spark wants *much* more than 4GB!
